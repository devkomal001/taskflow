...
const Members: React.FC = () => {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto transition-all duration-300 page-enter">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-[#286CFC]/12 dark:border-[#286CFC]/10 pb-5">
        ...
        {isAuthorized && (
          <button
            ...
            className="flex items-center gap-2 rounded-xl btn-brand px-4 py-2.5 text-sm font-bold shadow-lg shadow-[#286CFC]/20"
          >
            <UserPlus size={16} />
      ...
      <div className="rounded-2xl border border-[#286CFC]/12 dark:border-[#286CFC]/10 bg-white/10 dark:bg-[#07153D]/30 overflow-hidden shadow-sm backdrop-blur-md">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="border-b border-[#286CFC]/12 dark:border-[#286CFC]/10 bg-[#286CFC]/4 dark:bg-[#07153D]/60 text-slate-600 dark:text-slate-300 uppercase tracking-wider font-semibold">
                <th className="px-6 py-4 text-xs">Name & Email</th>
                <th className="px-6 py-4 text-xs">Workspace Role</th>
                <th className="px-6 py-4 text-xs">Status</th>
                {isAuthorized && <th className="px-6 py-4 text-right text-xs">Actions</th>}
              </tr>
            </thead>
            <tbody className="divide-y divide-[#286CFC]/8 dark:divide-[#286CFC]/6">
              {filteredMembers.length === 0 ? (
                <tr>
                  <td colSpan={isAuthorized ? 4 : 3} className="px-6 py-14 text-center">
                    <div className="flex flex-col items-center justify-center gap-3">
                      <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#286CFC]/8 text-[#286CFC]/60 border border-[#286CFC]/15">
                        <UserCheck size={22} className="stroke-[1.5]" />
                      </div>
                      <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">No team members match your search criteria</p>
                    </div>
                  </td>
                </tr>
              ) : (
                filteredMembers.map((member) => (
                  <tr key={member.id} className="hover:bg-[#286CFC]/4 dark:hover:bg-[#286CFC]/5 transition-all duration-150">
                    {/* Name and avatar */}
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3.5">
                        {member.profile.avatar_url ? (
                          <img src={member.profile.avatar_url} alt="" className="h-10 w-10 rounded-full object-cover border-2 border-[#286CFC]/20 shadow-sm" />
                        ) : (
                          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-tr from-[#286CFC]/15 to-[#4CB5D4]/15 text-[#286CFC] text-sm font-bold border border-[#286CFC]/25">
                            {(member.profile?.full_name || 'US').substring(0, 2).toUpperCase()}
                          </div>
                        )}
                        <div>
                          <p className="font-bold text-slate-800 dark:text-slate-100 text-sm">{member.profile.full_name}</p>
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-[10px] font-bold uppercase ${
                        member.role === 'owner' 
                          ? 'bg-amber-500/8 text-amber-500 border-amber-500/25' 
                          : member.role === 'manager'
                          ? 'bg-[#286CFC]/8 text-[#286CFC] border-[#286CFC]/25'
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-bold border ${
                        member.status === 'active' 
                          ? 'bg-emerald-500/8 text-emerald-500 border-emerald-500/25' 
                          : 'bg-orange-500/8 text-orange-500 border-orange-500/25 animate-pulse'
                      }`}>
                        <span className={`h-1.5 w-1.5 rounded-full ${member.status === 'active' ? 'bg-emerald-500' : 'bg-orange-500'}`}></span>
                        ...
                        {member.role !== 'owner' && member.user_id !== currentUser?.id && (
                          <div className="flex items-center justify-end gap-2.5">
                            <button
                              ...
                              className="rounded-xl border border-[#286CFC]/20 dark:border-[#286CFC]/15 bg-white dark:bg-[#07153D] px-3.5 py-1.5 text-xs font-semibold text-slate-600 dark:text-slate-300 hover:bg-[#286CFC]/8 hover:text-[#286CFC] hover:border-[#286CFC]/35 transition-all active:scale-[0.97]"
                            >
                            ...
                            <button
                              ...
                              title="Remove Member"
                              className="rounded-xl border border-slate-200 dark:border-slate-700/50 p-2 text-slate-400 dark:text-slate-500 hover:bg-rose-500/10 hover:text-rose-500 hover:border-rose-400/30 dark:hover:border-rose-500/25 transition-all active:scale-[0.95]"
                            >
                              <Trash2 size={14} />
                            </button>
                          </div>
      ...
      {isAuthorized && invitations.filter(i => i.status === 'Pending' || i.status === 'Expired').length > 0 && (
        <div className="rounded-2xl border border-[#286CFC]/12 dark:border-[#286CFC]/10 bg-white/10 dark:bg-[#07153D]/30 overflow-hidden shadow-sm backdrop-blur-md">
          <div className="flex items-center gap-2.5 px-6 py-4 border-b border-[#286CFC]/12 dark:border-[#286CFC]/10 bg-[#286CFC]/4 dark:bg-[#07153D]/60">
            <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-amber-500/10 text-amber-500">
              <Clock size={14} />
            </div>
            <h3 className="text-xs font-bold text-slate-600 dark:text-slate-200 uppercase tracking-wider">Pending Invitations</h3>
            <span className="rounded-full bg-amber-500/10 border border-amber-500/25 px-2 py-0.5 text-[10px] font-bold text-amber-500 ml-1">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="border-b border-[#286CFC]/8 dark:border-[#286CFC]/8 text-slate-500 dark:text-slate-400 uppercase tracking-wider font-semibold">
              ...
              <tbody className="divide-y divide-[#286CFC]/6 dark:divide-[#286CFC]/5">
                {invitations
                  .filter(i => i.status === 'Pending' || i.status === 'Expired')
                  .map((inv) => (
                    <tr key={inv.id} className="hover:bg-[#286CFC]/4 dark:hover:bg-[#286CFC]/5 transition-all duration-150">
                      <td className="px-6 py-3.5">
                        <div className="flex items-center gap-2.5">
                          <div className="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-tr from-[#286CFC]/12 to-[#4CB5D4]/12 text-[#286CFC] text-[10px] font-bold border border-[#286CFC]/20">
                            {inv.email.substring(0, 2).toUpperCase()}
                          </div>
                          <div>
                            <p className="font-bold text-slate-800 dark:text-slate-100 text-sm">{inv.email}</p>
                            <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">Invited by {inv.profile?.inviter_name || 'Admin'}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-3.5">
                        <span className="inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-[10px] font-bold uppercase bg-[#286CFC]/8 text-[#286CFC] border-[#286CFC]/25">
                          <Shield size={11} className="stroke-[2]" />
                          {inv.role}
                        </span>
                      </td>
                      <td className="px-6 py-3.5">
                        <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-bold border ${
                          inv.status === 'Pending'
                            ? 'bg-amber-500/8 text-amber-500 border-amber-500/25 animate-pulse'
                            : 'bg-slate-100 dark:bg-slate-800/40 text-slate-500 border-slate-200 dark:border-slate-700/50'
                        }`}>
                      ...
                      <td className="px-6 py-3.5 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            ...
                            className="flex items-center gap-1.5 rounded-xl border border-[#286CFC]/20 dark:border-[#286CFC]/15 bg-white dark:bg-[#07153D] px-3 py-1.5 text-[11px] font-semibold text-[#286CFC] hover:bg-[#286CFC]/8 hover:border-[#286CFC]/35 transition-all active:scale-[0.97]"
                          >
                            <Send size={12} />
                            Resend
                          </button>
