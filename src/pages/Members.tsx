    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] p-6 max-w-md mx-auto text-center text-slate-800 dark:text-white">
        ...
        <p className="text-xs text-slate-500 mt-2 max-w-xs leading-relaxed">
          Please select or create a workspace first to manage team members. You can create a workspace from the sidebar selector or click below to return to the Dashboard.
        </p>
        <button
          onClick={() => navigate('/')}
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto transition-colors duration-200">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-2xl font-extrabold tracking-tight text-slate-800 dark:text-white md:text-3xl">Workspace Directory</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400">View team directories, manage role permissions, and invite collaborators.</p>
        </div>
        {isAuthorized && (
          <button
      ...
      {/* Search Bar */}
      <div className="flex rounded-2xl border border-slate-200 dark:border-slate-800/80 bg-white dark:bg-slate-900/20 p-4 shadow-xs">
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-800/80 bg-white dark:bg-slate-900/20 overflow-hidden shadow-xs">
          ...
          <table className="w-full text-left border-collapse text-xs">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-850">
              {filteredMembers.map((member) => (
                <tr key={member.id} className="hover:bg-slate-50/40 dark:hover:bg-slate-900/20 transition-colors">
                  {/* Name and avatar */}
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-[10px] font-bold uppercase ${
                      member.role === 'owner' 
                        ...
                        : 'bg-slate-100 dark:bg-slate-850 text-slate-500 dark:text-slate-400 border-slate-250 dark:border-slate-800'
                    }`}>
                      <Shield size={10} />
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-bold border ${
                      ...
                      <span className={`h-1.5 w-1.5 rounded-full ${member.status === 'active' ? 'bg-emerald-500 dark:bg-emerald-400' : 'bg-orange-500 dark:bg-orange-400'}`}></span>
                      ...
                      {member.role !== 'owner' && member.user_id !== currentUser?.id && (
                        <div className="flex items-center justify-end gap-2">
                          <button
                            ...
                            className="rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-3 py-1.5 font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-850 dark:hover:text-white transition-colors"
                          >
                          ...
                          <button
                            ...
                            title="Remove Member"
                            className="rounded-lg border border-slate-200 dark:border-slate-850 p-1.5 text-slate-400 dark:text-slate-500 hover:bg-rose-500/10 hover:text-rose-600 dark:hover:text-rose-400 hover:border-rose-300 dark:hover:border-rose-500/20 transition-all duration-150"
                          >
                            <Trash2 size={14} />
                          </button>
                        </div>
          ...
          <div className="w-full max-w-md rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-2xl animate-in zoom-in-95 duration-200 text-slate-800 dark:text-slate-100">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                <UserPlus className="text-brand-500 dark:text-brand-400" size={20} />
            ...
            {inviteSuccess ? (
              <div className="my-6 text-center space-y-3">
                ...
                <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200">Invitation Sent!</h4>
                <p className="text-xs text-slate-500 dark:text-slate-400">An invitation email has been sent successfully.</p>
              </div>
            ) : (
              <form onSubmit={handleInvite} className="mt-4 space-y-4">
                {inviteError && (
                  <div className="flex items-center gap-2 rounded-lg border border-rose-500/20 bg-rose-500/5 p-3 text-[11px] font-semibold text-rose-605 dark:text-rose-400">
                ...
                <div>
                  ...
                  <div className="relative mt-1.5">
                    ...
                    <input
                      ...
                      className="w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 py-2.5 pl-10 pr-4 text-sm text-slate-800 dark:text-slate-200 placeholder-slate-450 dark:placeholder-slate-650 focus:border-brand-500/80 focus:outline-none transition-colors"
                    />
                  </div>
                </div>

                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Workspace Permission Role</label>
                  <select
                    value={inviteRole}
                    onChange={(e) => setInviteRole(e.target.value as any)}
                    className="mt-1.5 w-full rounded-xl border border-slate-200 dark:border-slate-855 bg-slate-50 dark:bg-slate-950 p-2.5 text-sm text-slate-800 dark:text-slate-200 focus:border-brand-500 focus:outline-none cursor-pointer"
                  >
                ...
                <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-200 dark:border-slate-800">
                  <button
                    type="button"
                    onClick={() => setIsInviteModalOpen(false)}
                    className="rounded-xl px-4 py-2.5 text-sm font-semibold text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-805 dark:hover:text-slate-200 transition-colors"
                  >
                    Cancel
                  </button>
                  <button
