...
const FirewallDashboard: React.FC = () => {
  ...
  const getChartData = () => {
    // Group audit logs by date (last 7 days)
  ...
  const getActionBadgeColor = (action: string) => {
    ...
    if (act.includes('success')) return 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20';
    if (act.includes('failed')) return 'bg-amber-500/10 text-amber-400 border-amber-500/20';
    if (act.includes('blocked')) return 'bg-rose-500/10 text-rose-400 border-rose-500/20';
    return 'bg-blue-500/10 text-brand-400 border-brand-500/20';
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-white">Application Security Firewall</h1>
          <p className="text-sm text-slate-400">Monitor failed authentication attempts, rate limits, and block suspicious client IPs.</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 rounded-xl bg-slate-900 border border-slate-800 px-3 py-1.5 text-xs text-slate-400">
            <Globe size={14} className="text-brand-400" />
            <span>Client IP: <strong className="text-slate-200">{clientIp}</strong></span>
          </div>
          <button
            onClick={loadData}
            disabled={loading}
            className="flex items-center gap-1.5 rounded-xl border border-slate-800 bg-slate-900/50 hover:bg-slate-900 px-4 py-2 text-xs font-semibold text-slate-200 transition-colors disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-2xl border border-slate-800 bg-slate-900/35 p-5 backdrop-blur-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Blocked IP Addresses</span>
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-rose-500/10 text-rose-400 border border-rose-500/20">
              <ShieldAlert size={16} />
            </div>
          </div>
          <p className="mt-2 text-3xl font-extrabold text-white">{totalBlocked}</p>
          <p className="text-[10px] text-slate-500 mt-1.5">Currently blacklisted from logins</p>
        </div>

        <div className="rounded-2xl border border-slate-800 bg-slate-900/35 p-5 backdrop-blur-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Failed Login Attempts</span>
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-amber-500/10 text-amber-400 border border-amber-500/20">
              <Shield size={16} />
            </div>
          </div>
          <p className="mt-2 text-3xl font-extrabold text-white">{failedLoginsCount}</p>
          <p className="text-[10px] text-slate-500 mt-1.5">Exceeding password triggers</p>
        </div>

        <div className="rounded-2xl border border-slate-800 bg-slate-900/35 p-5 backdrop-blur-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Blocked Requests</span>
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-red-500/10 text-red-400 border border-red-500/20">
              <ShieldOff size={16} />
            </div>
          </div>
          <p className="mt-2 text-3xl font-extrabold text-white">{blockedAttemptsCount}</p>
          <p className="text-[10px] text-slate-500 mt-1.5">Rate limit thresholds hit</p>
        </div>
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/35 p-5 backdrop-blur-sm">
          ...
          <p className="mt-2 text-3xl font-extrabold text-white">{totalAuditsCount}</p>
          <p className="text-[10px] text-slate-500 mt-1.5">Total audit logs recorded</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-6 lg:col-span-1">
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/25 p-5">
            ...
            <form onSubmit={handleManualBlock} className="space-y-4">
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500">IP Address</label>
                <input
                  id="block-ip-address"
                  type="text"
                  required
                  ...
                  className="mt-1.5 w-full rounded-xl border border-slate-850 bg-slate-950 p-2.5 text-xs text-slate-200 focus:border-brand-500/80 focus:outline-none"
                />
              </div>

              <div>
                <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500">Block Reason</label>
                <input
              ...
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500">Block Cooldown</label>
                <select
                  id="block-duration-select"
                  value={blockDuration}
                  onChange={(e) => setBlockDuration(e.target.value)}
                  className="mt-1.5 w-full rounded-xl border border-slate-850 bg-slate-950 p-2.5 text-xs text-slate-300 focus:border-brand-500/80 focus:outline-none"
                >
              ...
              <button
                ...
                className="flex w-full items-center justify-center gap-1.5 rounded-xl bg-rose-600 py-2.5 text-xs font-semibold text-white hover:bg-rose-500 transition-colors shadow-lg shadow-rose-600/20 disabled:opacity-50"
              >
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/25 p-5">
            <h3 className="text-sm font-bold uppercase tracking-wider text-slate-300 mb-4 flex items-center justify-between">
              ...
              <span className="rounded bg-rose-500/10 px-1.5 py-0.5 text-[10px] text-rose-400 font-semibold">{blockedIps.length}</span>
            </h3>

            {loading ? (
              <div className="py-6 text-center text-xs text-slate-500">Loading block lists...</div>
            ) : blockedIps.length === 0 ? (
              <div className="rounded-xl border border-dashed border-slate-850 p-6 text-center text-xs text-slate-500">
                No IP addresses are currently blocked.
              </div>
            ) : (
              <div className="space-y-3 max-h-96 overflow-y-auto pr-1">
                {blockedIps.map((ip) => (
                  <div key={ip.id} className="rounded-xl border border-slate-850 bg-slate-950/60 p-3 flex items-start justify-between gap-2">
                    <div className="overflow-hidden space-y-1">
                      <div className="flex items-center gap-1.5">
                        <span className="text-xs font-bold text-slate-300">{ip.ip_address}</span>
                        <span className="rounded bg-slate-900 border border-slate-800 px-1 py-0.5 text-[8px] text-slate-500">
                          {ip.attempts_count} hits
                        </span>
                      </div>
                      <p className="text-[10px] text-slate-400 truncate leading-relaxed">
                        Reason: <strong className="text-slate-300">{ip.reason}</strong>
                      </p>
                    ...
                    <button
                      ...
                      title="Unblock IP"
                      className="rounded-lg p-1.5 text-slate-500 hover:bg-slate-900 hover:text-emerald-400 transition-colors shrink-0"
                    >
                      <Trash2 size={13} />
                    </button>
                  </div>
        ...
        <div className="space-y-6 lg:col-span-2">
          {/* Chart Card */}
          <div className="rounded-2xl border border-slate-800 bg-slate-900/25 p-5">
            <h3 className="text-sm font-bold uppercase tracking-wider text-slate-300 mb-4">Security Incident Timeline</h3>
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/25 p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-4">
              <h3 className="text-sm font-bold uppercase tracking-wider text-slate-300">Security Audit Logs</h3>
              ...
              <div className="relative max-w-xs w-full">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-slate-850 bg-slate-950 py-1.5 pl-9 pr-4 text-xs text-slate-200 placeholder-slate-650 focus:border-brand-500/80 focus:outline-none transition-colors"
                />
              </div>
            </div>
            ...
            {loading ? (
              ...
              <div className="rounded-xl border border-dashed border-slate-850 p-12 text-center text-xs text-slate-500">
              ...
              <div className="overflow-x-auto">
                  ...
                  <tbody className="divide-y divide-slate-850 text-xs">
                    {filteredLogs.slice(0, 100).map((log) => (
                      <tr key={log.id} className="hover:bg-slate-900/20 transition-colors">
                        ...
                        <td className="py-3 pr-2 text-slate-300 max-w-[150px] truncate">{log.email || 'N/A'}</td>
                        <td className="py-3 pr-2 text-slate-300 font-mono">{log.ip_address}</td>
                        <td className="py-3 pr-2 text-slate-400 whitespace-nowrap">{new Date(log.created_at).toLocaleString()}</td>
                        <td className="py-3 text-[10px] text-slate-500 max-w-[180px] truncate" title={log.user_agent}>
                          {log.user_agent}
                        </td>
                      </tr>
                    ))}
                ...
                {filteredLogs.length > 100 && (
                  <p className="text-[10px] text-slate-500 text-center mt-3">Showing latest 100 audit entries.</p>
                )}
              </div>
            )}
          </div>
